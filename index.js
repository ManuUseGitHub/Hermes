// TODO: split this content into smaller components
...
const VUtils = require( "./ZiQuatorze/VerbsSuport/English/Utilities" );

const page = require( "./page" );
const EnglishVerbsCommand = require( "./ZiQuatorze/VerbsSuport/English/Commands" );
const internalize = new Internalize( Users );
const vutils = new VUtils( internalize );
const EnglishVCommand = new EnglishVerbsCommand( vutils );
...
// TODO: Create a module for role management only
...
// HACK: do something to better event manage clicks, messages, etc...
...
const validateNewUser = ( roleChanges ) => {
    ...
    if ( diff[ 0 ] === Roles.V_R.id && newRIds.length > oldRIDs.length ) {
        ...
        if ( lang ) {
            ...
            // MESSAGING THE USER
            const translatedMessage = internalize.tu(
                "accepted_adhesion" ,
                member._new
            );
...
client.on( "message" , ( message ) => {
    ...
    // check messages having the z13 command
    EnglishVCommand.setZ13MessagesCheck( message , Resources , Users );

    // TODO: Separate this
    if ( message.content == "z14 all" ) {
        if ( message.channel.id != _Guild.C_NEW_COMMERS.id ) {
            const response = internalize.tuid(
                "wrong_channel_for_command" ,
                message.author.id ,
                [ message.author.id , _Guild.C_NEW_COMMERS.id ]
            );
